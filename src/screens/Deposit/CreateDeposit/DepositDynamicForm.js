...
const DepositDynamicForm = ({ navigation, route }) => {
  // console.log('routes from dynamic deposite',route?.params)
  ...
  if( allValuesAdded.length == 0 ||allValuesAdded.length!=allRequiredLabels?.length || allValuesAdded.includes("")||allValuesAdded.includes(" ")||allValuesAdded.includes({}))
 {
  ...
  const submitDeposit = async (obj) => {
    ...
    try {
      ...
      axios({
        method: 'POST',
        url: baseURL + ENDPOINTS.createDepositRequest,
        headers: {
        ...
        .then((responseData) => {
          ...
          if (responseData.data.code == 202) {
            // setTrx(responseData.data.deposit.trx);
            ...
            navigation.navigate('DepositPreview', { data: responseData.data, description: route?.params?.details?.method?.description });
          }
          else {
            setSubmitLoading(false)
            dispatch(showSimpleModal({
                      ...
                      return (
                        <View>
                          ...
                          <Textarea
                            ...
                            style={[styles.textarea,{textAlign:i18n.language=='ar'?'right': 'left', color: COLORS.black,height:'100%',textAlignVertical:'top'}]}
                            onChangeText={(inputVal) => {
                              let input = [...allInputValues]
                              let inputIndex = input.findIndex(x => x.index === index)
                              if (inputIndex !== -1) {
                        ...
                        <>
                          <SelectDropdown
                            ...
                            onSelect={(inputVal, index) => {
                              ...
                              let input = [...filter]
                              let inputIndex = input.findIndex(x => x.index === index)
                              console.log('selectbug',input,inputIndex)
                              // if (inputIndex !== -1) {
                              //   input[inputIndex].label = label
                              //   input[inputIndex].val = inputVal
                              // }
...
export default DepositDynamicForm;
