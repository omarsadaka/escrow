...
const WithdrawDynamicForm = ({ navigation,route }) => {
//   console.log('routes from dynamic withdraw',route?.params)
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    let arr = []
    for (const prop in route?.params?.dynamicForm ) {
      arr.push(route?.params?.dynamicForm[prop])
    }
    // console.log('arr',arr)
    setFormData(arr)
    getBaseURL()?.then((res) => {
  ...
  const submitWithdraw = async(obj) => {
    let token = await AsyncStorage.getItem('TOKEN');
    const deviceInfo= await AsyncStorage.getItem('DeviceInfo')
    try {
        setSubmitLoading(true)
        // console.log('extractdata',obj)
       
     

    //     const finalReq = {...obj,
    //         method_code:route?.params?.method_code,
    //         amount:route?.params?.amount,
    //     }
    //     var formData = new FormData();
    //     for(let i in finalReq)
    //     {
    //         console.log(i,finalReq[i])
    //         formData.append(i,finalReq[i])
    //     }
    //    console.log('formData',formData)

    //     axios({
    //         method: "POST",
    //         url: baseURL + ENDPOINTS.withdrawRequest,
    //         headers: {
    ...
    //         .then((responseData) => {
       ...
       console.log('finalReq',finalReq)
        ...
        axios({
            ...
            .then((responseData) => {
                console.log('res code',responseData.data.code)
                if (responseData.data.code==202 || responseData.data.code==200) {
                 ...
                 navigation.navigate('WithdrawPreview',{data: responseData.data,description:route?.params?.details?.description});
                }
                else {
                    setSubmitLoading(false)
                    dispatch( showSimpleModal({
...
export default WithdrawDynamicForm;
